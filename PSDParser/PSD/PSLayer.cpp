...
int LayerRect::top() {
    return IntFromBytes((char *)_top, 4);
}

int LayerRect::left() {
    return IntFromBytes((char *)_left, 4);
}

int LayerRect::bottom() {
    return IntFromBytes((char *)_bottom, 4);
}

int LayerRect::right() {
    return IntFromBytes((char *)_right, 4);
}
...
void PSDLayerParser::startParse(ifstream *file, int version) {
    ...
    char bufSection[sizeOfBuffer];
    file->read(bufSection, sizeOfBuffer);
    lengthSec = IntFromBytes(bufSection, sizeOfBuffer);
    
    printf("Section Length ... %i\n\n", lengthSec);
    
    //// Layer info
    
    char bufInfo[sizeOfBuffer];
    file->read(bufInfo, sizeOfBuffer);
    lengthInfo = IntFromBytes(bufInfo, sizeOfBuffer);
    ...
    char bufNumLayer[2];
    file->read(bufNumLayer, 2);
    numLayer = SignedIntFromBytes(bufNumLayer, 2);
